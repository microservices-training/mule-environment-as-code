...
const supportedExternalPropertySources = {
    KEEPASS: {text: "keepass", ops: loadKeePassFile_kdbxweb}
    //,KEEPASS: {text: "keepass", ops: loadKeePassFile_keepassio}
};
...
function loadKeePassFile_kdbxweb(applications, callBack) {
    // Detect some needed paths
    var keePassFile = path.join(__dirname,'..','app_properties','keepass',process.env.keePassFileName);
    var keyFileName = path.join(__dirname,'..','app_properties','keepass',process.env.keyFileName);
    ...
    var keePassFileAsArryBuffer = fs.readFileSync(keePassFile, null).buffer;
    ...
    kdbxweb.Kdbx.load(keePassFileAsArryBuffer, credentials).then (db => {
        ...
        for (var eachApp in applications) {
            var app = applications[eachApp];
            var externalProperties = findProperty_kdbxweb(app.propertyKeys, entries);
            callBack(externalProperties, app);
        }
        console.log('--- Anypoint API: all the changes have been applied successfully');
    });
...
function findProperty_kdbxweb(propertyFilters, entries) {
  var consolidateProperty = {};
