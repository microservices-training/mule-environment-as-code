...
//load libraries
const repoType = require('./../../repo-type/repo-type');
 ...
 */
function deploy(externalProperties, application, ENV, ORGID) {

	console.log("\u001b[33m### Running deployment of application\u001b[39m: " + application.name);
	var cloudAppDetails = get_application_details(application.name, muleCommon.exec, ENV, ORGID);

	if(cloudAppDetails == null) { //trigger new application deployment
		console.log("Deploying: " + application.name);
		deploy_new_application(application, muleCommon.exec, externalProperties, ENV, ORGID);
		...
		redeploy_or_modify_application(application, muleCommon.exec, cloudAppDetails, externalProperties, ENV, ORGID);
	} else {
 ...
 */
function deploy_new_application(app, execSync, externalProperties, ENV, ORGID) {
	repoType.downloadPackage(app, muleCommon.exec);
 ...
 */
function redeploy_or_modify_application(app, execSync, cloudAppDetails, externalProperties, ENV, ORGID) {
	repoType.downloadPackage(app, muleCommon.exec);
